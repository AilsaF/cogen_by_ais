...
def format_time(seconds):
    ...
    if s < 60:
        ...
        return '%dd %dh %02dm' % (s / (24 * 60 * 60), (s / (60 * 60)) % 24, (s / 60) % 60)
...
def print_network_topology_info(layers):
    ...
    for l in lasagne.layers.get_all_layers(layers):
        ...
        act_str = '' if not hasattr(l, 'nonlinearity') else l.nonlinearity.__name__ if isinstance(l.nonlinearity,
                                                                                                  types.FunctionType) else type(
            l.nonlinearity).__name__
