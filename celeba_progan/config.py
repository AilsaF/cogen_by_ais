...
theano_flags[
    'warn_float64'] = 'warn'  # Give a warning when float64 ops are used by accident  (not sure if this actually works).
theano_flags[
    'assert_no_cpu_op'] = 'warn'  # Give a warning for CPU ops are used by accident (not sure if this actually works).
theano_flags['allow_gc'] = 'True'  # Conserve memory. Performance loss is alleviated by subsequent flags.
theano_flags[
    'gpuarray.preallocate'] = '0.90'  # Preallocate % of GPU memory to reduce cost at runtime (new gpuarray backend).
...
if 0:
    ...
    dataset = dict(h5_path='celeba-hq-1024x1024.h5', resolution=1024, max_labels=0, mirror_augment=True,
                   max_images=30000)
...
if 1:
    categories = ['airplane', 'bedroom', 'bicycle', 'bird', 'boat', 'bottle', 'bridge', 'bus', 'car', 'cat',
                  'chair', 'churchoutdoor', 'classroom', 'conferenceroom', 'cow', 'diningroom', 'diningtable', 'dog',
                  'horse', 'kitchen',
                  'livingroom', 'motorbike', 'person', 'pottedplant', 'restaurant', 'sheep', 'sofa', 'tower', 'train',
                  'tvmonitor']
    category_idx = 1
    ...
    dataset = dict(h5_path=h5_path, resolution=64, max_labels=0, mirror_augment=mirror_augment)
...
if 0:
    ...
    h5_paths = {'celeb128': '/home/tf6/DATA/celeba128.h5'}#, 'bedroom128': 'lsun-bedroom-256x256-full.h5'}
    mirror_augment = {'celeb128': True}#, 'bedroom128': False}
    dataset = dict(h5_path=h5_paths[datasets[dataset_idx]], max_labels=0, resolution=128,
                   mirror_augment=mirror_augment[datasets[dataset_idx]])
