...
def generate_data():
    data = Image.open('../sample_imgs/celeba_sample.png')
...
def main(aux, num_steps, temp_range):
    ...
    misc.save_image_grid(X_test*mask, os.path.join(result_subdir, 'trueBlocked.png'), drange=[-1, 1], grid_size=(1, 1))

    G, _, _ = misc.load_pkl('/home/tf6/cogen_AIS/model/128celeba-network-snapshot-{}.pkl'.format(aux))
    ...
    init_h = np.random.normal(0, 1, size=[10, hdim]).astype(np.float32)
    finalstate = samplers.run_ais(gen_fn, X_test, 10, num_steps, hdim, L, eps, init_state=init_h, save_dir=result_subdir,
                                  box=(box_pos[0], box_pos[0]+height, box_pos[1], box_pos[1]+width), temp_range=temp_range)


    post_img = gen_fn(finalstate).eval()
    misc.save_image_grid(post_img, result_subdir+'/fake.png', drange=[-1, 1], grid_size=(5, 2))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_aux", default="010000", type=str)
