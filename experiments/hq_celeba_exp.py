...
def generate_data(n):
    data = Image.open('../sample_imgs/celebaHQ_sample.png')
    data = np.asarray(data)
    data = data[:, :, :3]
    data = imresize(data, (128,128,3))
...
def main(aux, num_steps, temp_range):
    ...
    hdim = 512
    X_test = generate_data(imageId)
    misc.save_image_grid(X_test, os.path.join(result_subdir, 'true.png'), drange=[-1, 1], grid_size=(1, 1))

    G, D, _ = misc.load_pkl('/home/tf6/cogen_AIS/model/celeba-network-snapshot-{}.pkl'.format(aux))
    ...
    init_state = np.random.normal(0, 1, size=[3, hdim]).astype(np.float32)

    finalstate = samplers.run_ais(gen_fn, X_test, 3, num_steps, hdim, L, eps, init_state=init_state,
                                  save_dir=result_subdir, box=(0, 0, 0, 0), temp_range=temp_range)

    post_img = gen_fn_HQ(finalstate).eval()
    misc.save_image_grid(post_img, result_subdir+'/fake.png', drange=[-1, 1],
                         grid_size=(3,1))

    # calculate ssim
    all_ssim = []
    t = Image.open('../sample_imgs/celebaHQ_sample.png')
    t = np.array(t)
    f = Image.open(result_subdir + '/fake.png')
    f = np.array(f)

    for i in range(3):
        f1 = f[:, 1024 * i: 1024 * (i + 1)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_aux", default="019000", type=str)
    ...
    parser.add_argument("--imageId", default=5, type=int)
